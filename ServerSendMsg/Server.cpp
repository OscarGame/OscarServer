...
int main()
{
	...
	**/
	...
	//HANDLE ThreadHandle = 
	CreateThread(NULL, 0, ServerWorkThread, completionPort, 0, NULL);
	...
	while (true) {
		...
		DWORD RecvBytes;
		DWORD Flags = 0;
		int ret = WSARecv(PerHandleData->socket, &(PerIoData->databuff), 1, &RecvBytes, &Flags, &(PerIoData->overlapped), NULL);
